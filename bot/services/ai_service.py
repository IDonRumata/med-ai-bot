    ...
    content = [
        {
            ...
            "text": (
                ...
                f"summary (краткая сводка: что в норме, что ОТКЛОНЕНИЕ — выдели отклонения отдельно). "
                f"ВАЖНО для поля metric: используй ТОЛЬКО короткое стандартное русское название без латыни, "
                f"без кодов (#NEUT, %EOS, /ESR и т.д.), без дублирования (только одно название): "
                f"'ТТГ' (не 'Тиреотропный гормон (ТТГ)'), 'СОЭ' (не 'СОЭ по Вестергрену/ESR'), "
                f"'Нейтрофилы абс.' (не 'Нейтрофилы абс./#NEUT'), 'Железо' (не 'Сывороточное железо / Serum iron'), "
                f"'Эозинофилы абс.' (не 'Эозинофилы абс./#EOS'), 'Триглицериды', 'Холестерин', 'АЛТ', 'АСТ'. "
                f"Если показатель встречается несколько раз с разными названиями — оставь один.{profile_ctx}"
            ),
        }
    ]
